...
> **✅ Выполнено 2026-07-20** — коммит `a5189aa`. 161 server тест зелёный, server+client typecheck чистые. Секрет в БД (миграция), клиент хранит токен в localStorage; заклеймённый ник без верного токена отклоняется.
...
> **✅ Выполнено 2026-07-20** — коммит `ee96b5f`. Реализовано чище, чем эскиз ниже: без изменения `PlayerRecord`/`save()`, а отдельными `db.getRentDue`/`db.setRentDue` (колонка `rent_due`), персист в `savePlayer`, восстановление в `onJoin`. 163 теста зелёные.
...
> **✅ Выполнено 2026-07-20** — коммит `dd5e847`. `COP_PATROL_MIN_DIST=30`, якорь патруля в `Runtime`, гейт зарплаты по смещению. 198 тестов зелёные (34 shared + 164 server).
...
> **✅ Этап B выполнен 2026-07-21** (ветка `hardening-b`). 170 server + 34 shared тестов зелёные, typecheck чистый по всем воркспейсам.
> - B6 typecheck+CI — `2c3b28e`
> - B1 ложный отказ перевода — `a863d61`
> - B4 целые деньги + rate-limit — `14a21b8`
> - B5 выход из машины через коллизию — `b58e00b`
> - B3 версия протокола (мягкая к отсутствию ver, строгая к несовпадению) — `28c703c`
> - B2 уникальные ники + заморозка призрака — `c6ada87`
